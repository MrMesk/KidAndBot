...
public class YuME_mapEditor : EditorWindow
{
    ...
    static void updateGridScale()
    {
        if (gridSceneObject != null)
        {
            try
            {
                gridSceneObject.GetComponent<YuME_GizmoGrid>().tileSizeX = globalScale;
            }
            catch
            {
                gridSceneObject.GetComponent<YuME_GizmoGrid>().tileSizeX = 1f;
            }

            try
            {
                gridSceneObject.GetComponent<YuME_GizmoGrid>().tileSizeZ = globalScale;
            }
            catch
            {
                gridSceneObject.GetComponent<YuME_GizmoGrid>().tileSizeZ = 1f;
            }
        }
    ...
    static void updateSceneMousePosition()
    {
        ...
        {
            tilePosition.x = Mathf.Round((hit.point.x - hit.normal.x * 0.001f) / globalScale) * globalScale;
            tilePosition.z = Mathf.Round((hit.point.z - hit.normal.z * 0.001f) / globalScale) * globalScale;
            tilePosition.y = gridHeight;
            //Debug.Log("Hit.point: " + hit.point + "Tile Position:" + tilePosition);
        }
    }
