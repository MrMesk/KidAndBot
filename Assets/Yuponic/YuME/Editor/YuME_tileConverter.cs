...
class YuME_tileConverter : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        userSettings.c_tileStatic = EditorGUILayout.Toggle("Set Tiles as Static", userSettings.c_tileStatic);
        ...
        userSettings.c_shadowCastingMode = EditorGUILayout.Popup("Cast Shadows", userSettings.c_shadowCastingMode, new string[] { "Off", "On", "ShadowsOnly", "TwoSided" });

        userSettings.c_receiveShadows = EditorGUILayout.Toggle("Receive Shadows", userSettings.c_receiveShadows);
        ...
        userSettings.bypassCollisionSetup = EditorGUILayout.Toggle("Add Mesh Collider", userSettings.bypassCollisionSetup);
    ...
    static void createTileSet()
    {
		...
		foreach (GameObject child in sourcePrefabs)
        {
			...
			{
				validTile = convertPrefab(child.gameObject, child.name);
			}
			else
			{
	            foreach (Transform subChild in child.transform)
	            {
					validTile = convertPrefab(subChild.gameObject, child.name);
	            }
			}
	...
	static bool convertPrefab(GameObject tile, string name)
	{
		if (tile.GetComponent<MeshRenderer>() != null)
		{
			GameObject newTile = (GameObject)Object.Instantiate(tile);

			MeshRenderer childMeshRenderer = newTile.GetComponent<MeshRenderer>();

			if (childMeshRenderer == null && !userSettings.c_bypassCollisionSetup)
			{
			...
			tileParentObject.name = userSettings.appendName + name;
            tileParentObject.AddComponent<YuME_tileGizmo>();

            newTile.transform.parent = tileParentObject.transform;
            newTile.transform.position = Vector3.zero + userSettings.c_offset;
            newTile.isStatic = userSettings.c_tileStatic;
            ...
            Object prefabAlreadyCreated = AssetDatabase.LoadAssetAtPath(userSettings.c_destinationFolder + "/" + userSettings.appendName + name + "_YuME.prefab", typeof(GameObject));
    ...
    {
        //tilesetData = AssetDatabase.LoadAssetAtPath(userSettings.destinationFolder + "/" + userSettings.sourceTiles.name + ".asset", typeof(YuME_tilesetData)) as YuME_tilesetData;

        Debug.Log("Yuponic Tile Importer finished: Updated " + updatedPrefabsCounter + " prefabs and Created " + newPrefabsCounter + " prefabs in " + userSettings.c_destinationFolder);
        //DestroyImmediate(tileParentObject);
    }
}
